...
// Mock constants to override API KEY
jest.mock('@/config/constants', () => {
  const originalModule = jest.requireActual('@/config/constants');
  return {
    ...originalModule,
    APP_CONFIG: {
      ...originalModule.APP_CONFIG,
      GOOGLE_MAPS: {
        ...originalModule.APP_CONFIG.GOOGLE_MAPS,
...
describe('Home Component', () => {
  ...
  it('renders all essential UI elements', () => {
    ...
    expect(
      screen.getByRole('button', { name: /ルートを生成/i }),
    ).toBeInTheDocument();
  });
  ...
  it('successfully fetches and displays route data', async () => {
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/v1/route/generate'),
        expect.anything(),
      );
    });
  ...
  it('displays an error message when the API call fails', async () => {
    ...
    const errorMsg = await screen.findAllByText(/ルート検索に失敗しました/);
  ...
  it('displays a network error message when fetch rejects', async () => {
    ...
    const errorMsg = await screen.findAllByText(/Network failed/);
    expect(errorMsg.length).toBeGreaterThan(0);
  });
  ...
  it('shows loading state during API call', async () => {
    ...
    await waitFor(
      () => {
        ...
        const searchingButton = buttons.find(
          (b) =>
            b.textContent?.includes('Searching') ||
            b.textContent?.includes('検索中'),
        );
  ...
  it('sends correct parameters including distance and elevation', async () => {
    ...
    const distanceInput = screen.getByLabelText(
      /Target Distance|希望走行距離/i,
    );
    fireEvent.change(distanceInput, { target: { value: '50' } });

    const elevationInput = screen.getByLabelText(
      /Target Elevation|希望獲得標高/i,
    );

...
public:
    MOCK_METHOD(void, getElevation, (const services::Coordinate& coord, ElevationCallback&& callback), (override));
    MOCK_METHOD(void, getElevations, (const std::vector<services::Coordinate>& coords, ElevationsCallback&& callback), (override));
    MOCK_METHOD(std::optional<double>, getElevationSync, (const services::Coordinate& coord), (override));
};
...
// checkAndQueueRefresh involves async call on event loop, hard to test synchronously without full Drogon setup.
// We can test the logic flow if we could intercept the loop.
// For unit test, we might skip the async part or rely on Integration test.
